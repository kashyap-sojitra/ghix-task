      ...
      <div className="flex-1 max-w-3xl mx-auto w-full px-4 py-8">
        ...
        {inputSnap && (
          <div className="flex items-center gap-2 flex-wrap mb-5">
            {Boolean(
              inputSnap.origin_country && inputSnap.destination_country,
            ) && (
              <span className="bg-slate-800 text-slate-100 text-xs font-bold px-3 py-1.5 rounded-full capitalize">
                {String(inputSnap.origin_country)} →{" "}
                {String(inputSnap.destination_country).replace(/-/g, " ")}
              </span>
            )}
            ...
            {Boolean(inputSnap.salary_expectation) && (
              <span className="bg-emerald-100 text-emerald-800 text-xs font-bold px-3 py-1.5 rounded-full">
                {String(inputSnap.salary_currency)}{" "}
                {Number(inputSnap.salary_expectation).toLocaleString()}
              </span>
            )}
        ...
        {error && (
          <div className="bg-red-50 border-2 border-red-200 text-red-800 rounded-2xl px-5 py-4 text-sm font-medium">
            {error}
          </div>
        )}
        ...
        {plan && plan.outputSnapshot != null && (
          <PlanResult
            data={{ data: plan.outputSnapshot as Record<string, unknown> }}
          />
        )}
      </div>
    </div>
