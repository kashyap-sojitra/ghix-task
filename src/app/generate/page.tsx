...
const schema = z.object({
  ...
  work_authorisation_constraint: z.enum([
...
const inputClass =
  "w-full border-2 border-slate-200 rounded-lg px-3 py-2.5 text-slate-900 text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500 transition-colors bg-white";
const selectClass =
  "w-full border-2 border-slate-200 rounded-lg px-3 py-2.5 text-slate-900 text-sm focus:outline-none focus:border-blue-500 transition-colors bg-white";
...
export default function GeneratePage() {
  ...
  useEffect(() => {
    destinations
      .list()
      .then((res) => {
        const index = res.data as {
          data: { supported_combinations: DestinationOption[] };
        };
  ...
  useEffect(() => {
    ...
    const dest = destinationOptions.find(
      (d) => d.destination_slug === selectedDestination,
    );
    if (dest) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await plans.save({
        ...
        output_snapshot: (result as { data: Record<string, unknown> })
  ...
  const workAuthOptions = [
    {
      value: "needs_employer_sponsorship",
      label: "Needs employer sponsorship",
    },
    { value: "no_constraint", label: "No constraint — open to any route" },
    {
      value: "already_has_right_to_work",
      label: "Already has right to work in destination",
    },
  ];
      ...
      <div className="flex-1 max-w-3xl mx-auto w-full px-4 py-8">
        <div className="mb-6">
          ...
          <p className="text-slate-500 text-sm mt-1">
            Fill in your career profile to get a personalised, data-backed plan
            with honest feasibility checks.
          </p>
        </div>

        <form
          onSubmit={handleSubmit(onSubmit)}
          className="bg-white border-2 border-slate-100 rounded-2xl p-6 shadow-sm space-y-6"
        >
          <div>
              ...
              <div>
                ...
                >
                  ...
                  {destinationOptions.map((d) => (
                    <option key={d.destination_slug} value={d.destination_slug}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className={`${selectClass} ${availableRoles.length === 0 ? "opacity-50" : ""}`}
            ...
            <div>
              ...
              {!watch("salary_currency") && (
                <p className="text-slate-400 text-xs mt-1.5">
                  Currency auto-fills when you select a destination.
                </p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid gap-2">
              {workAuthOptions.map((opt) => (
                <label
                  key={opt.value}
                  className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border-2 border-slate-100 hover:border-blue-200 hover:bg-blue-50 transition-colors has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50"
                >
                  <input
                    type="radio"
                    value={opt.value}
                    {...register("work_authorisation_constraint")}
