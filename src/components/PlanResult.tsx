...
const cardClass =
  "bg-white border-2 border-slate-100 rounded-2xl p-6 shadow-sm";
const sectionLabel =
...
const feasibilityConfig: Record<
  string,
  { border: string; bg: string; text: string; dot: string; icon: string }
> = {
  feasible: {
...
function ConfidenceBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`${cfg.bg} ${cfg.text} text-xs font-bold px-2.5 py-1 rounded-full`}
    >
      {cfg.label}
    </span>
  );
...
export default function PlanResult({
  ...
  const fCfg =
    feasibilityConfig[feasibility.score] ?? feasibilityConfig.feasible;
  const demand =
    demandConfig[market_demand.level.toLowerCase()] ?? demandConfig.medium;
    ...
    <div className="space-y-4">
      <div className={`border-2 ${fCfg.border} ${fCfg.bg} rounded-2xl p-5`}>
        <div className="flex items-center gap-3 mb-1">
          <div
            className={`w-8 h-8 ${fCfg.dot} rounded-full flex items-center justify-center`}
          >
          ...
          <div>
            ...
            <span className={`text-sm ml-2 ${fCfg.text} opacity-70`}>
              overall feasibility
            </span>
          </div>
        ...
        {feasibility.warnings.length > 0 && (
          <ul
            className={`mt-3 space-y-1.5 pt-3 border-t border-current border-opacity-20`}
          >
            {feasibility.warnings.map((w, i) => (
              <li
                key={i}
                className={`text-sm ${fCfg.text} flex items-start gap-2`}
              >
      ...
      {narrative_summary ? (
        ...
        meta.llm_status !== "success" && (
          <div className="bg-slate-100 border-2 border-slate-200 rounded-2xl p-4 text-sm text-slate-500 font-medium">
            Narrative unavailable ({meta.llm_status}) — plan data below is
            complete.
          </div>
        )
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className={cardClass}>
          <SectionHeader
            title="Visa Routes"
            confidence={data_confidence.fields.work_authorisation}
          />
          <div className="space-y-2.5">
            {eligible_routes.map((route, i) => (
              <div
                key={i}
                className="p-3 bg-slate-50 rounded-xl border-2 border-slate-100"
              >
                ...
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  <span className="text-xs text-slate-500 font-medium">
                    {route.processing_time_months.min}–
                    {route.processing_time_months.max} mo
                  </span>
        ...
        <div className={cardClass}>
          <SectionHeader
            title="Market Demand"
            confidence={data_confidence.fields.market_demand}
          />
          <div
            className={`inline-flex items-center gap-2 ${demand.bg} ${demand.text} px-3 py-1.5 rounded-full mb-3`}
          >
      ...
      <div className={cardClass}>
        <SectionHeader
          title="Salary Assessment"
          confidence={data_confidence.fields.salary}
        />
        <div className="grid grid-cols-3 gap-3 mb-4">
          <div className="text-center bg-slate-50 rounded-xl p-4 border-2 border-slate-100">
            ...
            <p className={statValue}>
              {salary_assessment.currency}{" "}
              {salary_assessment.user_expectation.toLocaleString()}
            </p>
          </div>
          <div className="text-center bg-blue-50 rounded-xl p-4 border-2 border-blue-100">
            ...
            <p className="text-lg font-bold text-blue-900">
              {salary_assessment.currency}{" "}
              {salary_assessment.market_median.toLocaleString()}
            </p>
          </div>
          <div className="text-center bg-slate-50 rounded-xl p-4 border-2 border-slate-100">
            ...
            <p className={statValue}>
              {salary_assessment.currency}{" "}
              {salary_assessment.minimum_threshold.toLocaleString()}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {salary_assessment.shortfall_from_median != null && (
            <span className="text-slate-500 text-xs font-medium">
              {salary_assessment.currency}
              {salary_assessment.shortfall_from_median.toLocaleString()} below
              median
            </span>
          )}
        </div>
      ...
      <div className={cardClass}>
        <SectionHeader
          ...
          confidence={data_confidence.fields.timeline}
        />
        <div className="grid grid-cols-3 gap-3 mb-3">
          {[
            {
              label: "Job search",
              min: timeline_breakdown.hiring_phase_months.min,
              max: timeline_breakdown.hiring_phase_months.max,
            },
            {
              label: "Visa processing",
              min: timeline_breakdown.visa_processing_months.min,
              max: timeline_breakdown.visa_processing_months.max,
            },
            {
              label: "Total estimate",
              min: timeline_breakdown.total_estimated_months.min,
              max: timeline_breakdown.total_estimated_months.max,
            },
          ].map((t) => (
            <div
              key={t.label}
              className="text-center bg-slate-50 rounded-xl p-4 border-2 border-slate-100"
            >
        ...
        <div
          className={`flex items-center gap-2 text-sm font-semibold rounded-lg px-3 py-2 ${timeline_breakdown.fits_user_timeline ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-800"}`}
        >
          <span>{timeline_breakdown.fits_user_timeline ? "✓" : "⚠"}</span>
          <span>
            {timeline_breakdown.fits_user_timeline
              ? "Fits within your stated timeline"
              : "Tight — consider extending your timeline"}
          </span>
        </div>
      </div>
              ...
              <li key={step.rank} className="flex gap-4">
                ...
                <div className="flex-1 pt-0.5">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span
                      className={`text-xs font-bold px-2 py-0.5 rounded-full ${pCfg.bg} ${pCfg.text}`}
                    >
                      {step.priority}
                    </span>
